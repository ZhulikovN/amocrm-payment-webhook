...
def get_purchase_count_mapping() -> dict[int, int]:
    """
    Возвращает маппинг ID количества купленных курсов из amoCRM в числовое значение.

    Ключ: enum_id из поля 'Купленных курсов' в amoCRM
    Значение: количество курсов (1-10)

    Returns:
        dict[int, int]: Словарь маппинга количества курсов
    """
    return {
        settings.AMO_PURCHASE_COUNT_1: 1,
        settings.AMO_PURCHASE_COUNT_2: 2,
        settings.AMO_PURCHASE_COUNT_3: 3,
        settings.AMO_PURCHASE_COUNT_4: 4,
        settings.AMO_PURCHASE_COUNT_5: 5,
        settings.AMO_PURCHASE_COUNT_6: 6,
        settings.AMO_PURCHASE_COUNT_7: 7,
        settings.AMO_PURCHASE_COUNT_8: 8,
        settings.AMO_PURCHASE_COUNT_9: 9,
        settings.AMO_PURCHASE_COUNT_10: 10,
    }


def map_purchase_count_to_number(purchase_count_enum_id: int) -> int:
    """
    Преобразует enum_id количества курсов из amoCRM в числовое значение.

    Args:
        purchase_count_enum_id: enum_id из поля 'Купленных курсов' в amoCRM

    Returns:
        int: Количество курсов (1-10)

    Raises:
        ValueError: Если маппинг для количества курсов не найден
    """
    mapping = get_purchase_count_mapping()

    if purchase_count_enum_id not in mapping:
        raise ValueError(f"Маппинг для количества курсов с enum_id {purchase_count_enum_id} не найден")

    return mapping[purchase_count_enum_id]
